...
async def test_incomplete_message_requests_missing_info() -> None:
    # Test 1: Mensaje incompleto (Caso 1)
    ...
    message = _incoming_message("Tengo cucarachas", external_user_id="user-t1")
...
async def test_partially_complete_message_requests_remaining_data() -> None:
    # Test 2: Mensaje parcialmente completo (Caso 2)
    # Debe solicitar el dato restante (contacto).
    firestore = MockFirestoreService()
    service = ConversationService(firestore_service=firestore)
    message = _incoming_message("Tengo cucarachas en la cocina del Bar Pepe.", external_user_id="user-t2")
...
async def test_complete_message_proposes_create_incident() -> None:
    # Test 3: Información completa (Caso 3)
    # Debe proponer la creación de incidencia.
    firestore = MockFirestoreService()
    service = ConversationService(firestore_service=firestore)
    message = _incoming_message("Tengo cucarachas en la cocina del Bar Pepe. Mi nombre es Pedro.", external_user_id="user-t3")
    ...
    assert response.incident.location == "cocina del Bar Pepe"
...
async def test_multi_turn_flow_reconstructs_state_and_creates_incident() -> None:
    ...
    # Mensaje 2: "Es en la cocina de mi bar"
    # Mensaje 3: "Soy Pedro"
    ...
    # Turno 2
    msg2 = _incoming_message("Es en la cocina de mi bar", external_user_id=user_id)
    resp2 = await service.handle_incoming_message(msg2)
    assert resp2.action.type == "collect_missing_data"

    # Turno 3
    msg3 = _incoming_message("Soy Pedro", external_user_id=user_id)
    ...
    assert resp3.incident.location == "cocina de mi bar"
    ...
    assert incident_in_db["location"] == "cocina de mi bar"
...
async def test_incident_creation_generates_audit_events() -> None:
    ...
    firestore = MockFirestoreService()
    service = ConversationService(firestore_service=firestore)
    message = _incoming_message("Tengo cucarachas en la cocina del Bar Pepe. Mi nombre es Pedro.", external_user_id="user-t5")
