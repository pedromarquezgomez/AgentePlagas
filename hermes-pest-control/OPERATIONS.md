...
`Acciones IA` is the operator review queue for proposed tool calls generated by
the controlled Nous Hermes PoC or future tool harness integrations.
...
Allowed review updates:

```json
{
  "review_status": "approved",
  "reviewer_notes": "Reviewed as proposal only.",
  "reviewed_by": "operator@example.com",
  "approved_payload": {}
}
```

Important: approval is not execution. Approval only changes the review state.
Sprint 25 adds a separate controlled execution action for `gmail.create_draft`
only. Gmail send, Calendar, Telegram, WhatsApp, and direct Firestore actions
remain disabled.

### Ejecucion controlada Gmail draft
...
1. `approved_payload` if present.
2. Otherwise the original proposal payload stored in metadata.
...
GMAIL_DRAFT_EXECUTION_ENABLED=false
...
Use a controlled Google account for Hermes, not a personal account. In Google
Workspace setups, service-account domain-wide delegation may be required and
`GMAIL_DELEGATED_USER` should point to the controlled mailbox.

The execution result stores safe identifiers such as `draft_id` and
`message_id`; it should not store the full email body. The draft is not sent.

#### Configuracion Gmail piloto

Tipo de credencial soportado actualmente:

- Service account JSON.
- Para Google Workspace, usar domain-wide delegation y delegar en la cuenta
  operativa de Hermes con `GMAIL_DELEGATED_USER`.
- No usar cuenta personal para pruebas operativas.
...
GMAIL_TOOLS_ENABLED=true
GMAIL_DRAFT_EXECUTION_ENABLED=true
GMAIL_CREDENTIALS_PATH=/ruta/fuera/del/repo/hermes-gmail-service-account.json
GMAIL_DELEGATED_USER=operaciones@tu-dominio.example
```

Cloud Run piloto:

1. Guardar el JSON en Secret Manager.
2. Inyectarlo como `GMAIL_CREDENTIALS_JSON` o montar ruta segura y usar
   `GMAIL_CREDENTIALS_PATH`.
3. Configurar:
...
GMAIL_TOOLS_ENABLED=true
GMAIL_DRAFT_EXECUTION_ENABLED=true
GMAIL_DELEGATED_USER=<cuenta-operativa-hermes>
```

No imprimir ni commitear el JSON. No poner credenciales en `.env.example`.

#### Comprobacion Gmail

Validar autenticacion sin crear borrador:

```bash
make gmail-draft-check
```

Crear borrador de prueba solo con confirmacion explicita:

```bash
CONFIRM_CREATE_GMAIL_DRAFT=true \
GMAIL_TEST_DRAFT_RECIPIENT=destino-controlado@example.test \
make gmail-draft-check
```

El script imprime `draft_id` y `message_id`, pero no imprime tokens ni el cuerpo
completo.

#### Prueba desde Acciones IA

Crear una accion IA aprobada de prueba:

```bash
CONFIRM_SEED_TOOL_RECORDS=true \
...
1. Abrir `/tools/executions`.
2. Abrir el registro `Borrador de email`.
3. Verificar que esta `Aprobada`.
4. Pulsar `Crear borrador en Gmail`.
5. Confirmar `executed=true`.
6. Confirmar `execution_result.draft_id`.
7. Revisar manualmente Gmail: debe existir el borrador y no debe haber correo
   enviado.
...
### Seed local de Acciones IA

Para probar la pantalla con datos controlados:

```bash
make seed-tool-executions
```

Con `MockFirestore`, los datos viven en memoria del proceso que ejecuta el
script. Esto sirve para validar el seed y los tests. Para ver los registros en
un panel conectado a un backend separado, usa Firestore real/emulador en un
entorno piloto controlado.

El seed crea ejemplos de:

- Gmail draft permitido como borrador.
- Gmail `send_email` denegado.
- Calendar `propose_event` pendiente de aprobacion.
- Telegram draft revisable.
- WhatsApp con consejo quimico denegado.
- Firestore direct write denegado.

Si el backend detecta Firestore real, el script se detiene salvo que confirmes
explicitamente:

```bash
CONFIRM_SEED_TOOL_RECORDS=true make seed-tool-executions
```

No uses esa confirmacion en produccion operativa salvo que quieras crear datos
piloto visibles en el panel.

## Hermes Agent Pilot Mode

Pilot Mode permite que Hermes Agent actue como respuesta primaria solo para
casos simples y de bajo riesgo. Esta desactivado por defecto y no cambia
`HERMES_MODE=mock` como base de rollback.

Variables:

```bash
HERMES_MODE=mock
HERMES_API_URL=https://<hermes-agent-url>/agent
...
La compuerta permite casos claros como:
...
- producto quimico, venenos, mezclas o dosis;
- mascotas, bebes, ninos, mayores, embarazadas o personas vulnerables;
- restaurantes, bares o negocio alimentario;
- cliente muy enfadado, reclamacion o amenaza legal;
- precio exacto o precio cerrado;
- mensajes ambiguos o incompletos.

Auditoria:

- `DecisionRecord.metadata.pilot_used=true` si se uso el agente.
- `DecisionRecord.metadata.pilot_blocked=true` si el gate lo bloqueo.
- `DecisionRecord.metadata.pilot_policy_rule` indica la regla aplicada.
- `DecisionRecord.metadata.pilot_risk_flags` lista las senales de riesgo.
...
No activar Pilot Mode en Cloud Run hasta revisar evals LLM, synthetic shadow,
DecisionRecords y HumanReview con trafico controlado.
